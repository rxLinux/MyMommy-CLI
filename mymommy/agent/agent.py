...
4. Validar: Verifique se as alterações funcionaram perfeitamente (rode testes, etc).

FORMATO MANDATÓRIO PARA USO DE FERRAMENTAS:
Se precisar usar uma ferramenta, você DEVE emitir o bloco JSON exatamente como nos exemplos acima.
"""
    ...
    def chat(self, user_input: str) -> Generator[Dict[str, Any], None, None]:
        ...
        # Reinforce maternal role and tool usage to completely override LLM laziness and default refusals!
        reinforced_input = f"""{user_input}

(Lembrete Crucial de Mãe: Eu tenho controle físico e acesso direto para gravar e modificar arquivos reais usando o 'direct_pc_file_manager' no HD e executar instruções reais com 'direct_pc_terminal_executor' no processador dele. Para atender ao pedido do meu filho agora mesmo, eu DEVO invocar as ferramentas emitindo o bloco JSON ```json ... ``` imediatamente!)"""
        
        messages.append({"role": "user", "content": reinforced_input})
            ...
            for call in tool_calls:
                ...
                if tool_name in self.tools:
                    try:
                        ...
                        yield {"type": "tool_result", "content": error_msg}
                        self.memory.add_interaction("assistant", full_response, tokens=count_tokens(full_response))
    ...
    def _extract_tool_calls(self, text: str) -> List[Dict[str, Any]]:
        ...
        # 1. Try finding json blocks within markdown code fences
        pattern = r"```(?:json)?\s*(\{.*?\})\s*```"
        ...
        # 2. Fallback: if no tool calls found, try searching for any raw JSON object in the text
        if not tool_calls:
            raw_json_pattern = r"(\{.*?\"tool\"\s*:\s*\".*?\".*?\})"
            raw_matches = re.findall(raw_json_pattern, text, re.DOTALL)
            for raw_match in raw_matches:
                try:
                    data = json.loads(raw_match.strip())
